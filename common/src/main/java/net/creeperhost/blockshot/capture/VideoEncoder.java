...
public class VideoEncoder implements Encoder {
    ...
    private final ExecutorService RECORDING_EXECUTOR = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() / 2, new ThreadFactoryBuilder().setNameFormat("blockshot-recorder-%d").setDaemon(true).build());
    ...
    public void uploadAndAddToChat(boolean writeOnFail, String fallbackExt, @Nullable AtomicDouble progress) {
        ...
        if (result == null) {
            ...
            if (Config.INSTANCE.copyToClipboard) {
                ...
                link.append(ChatFormatting.RESET + " ").append(Component.translatable("chat.blockshot.upload.copied").withStyle(ChatFormatting.GRAY));
            }
