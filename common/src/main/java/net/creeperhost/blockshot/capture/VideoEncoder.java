...
public class VideoEncoder implements Encoder {
        ...
        if (result == null) {
            ...
            if (writeOnFail) {
                try (FileInputStream is = new FileInputStream(tempFile)){
                    ...
                    IOUtils.copy(is, os);
                    ScreenshotHandler.saveLocal(os.toByteArray(), Platform.getGameFolder().toFile(), null, fallbackExt, ClientUtil::sendMessage, "chat.blockshot.fallback.success", "chat.blockshot.fallback.failure");
                } catch (IOException e) {
    ...
    public String uploadImage(File file, @Nullable AtomicDouble progress) {
        try (InputStream is = new FileInputStream(file)){
            ByteArrayOutputStream os = new ByteArrayOutputStream();
            IOUtils.copy(is, os);
            String rsp = WebUtils.post("https://blockshot.ch/upload", Base64.getEncoder().encodeToString(os.toByteArray()), MEDIA_TYPE, progress);
    ...
    public List<Component> getHudText() {
        ...
        if (stopping && !activeFutures.isEmpty()) {
            list.add(new TranslatableComponent("overlay.blockshot.encoding", activeFutures.size()).withStyle(ChatFormatting.RED));
