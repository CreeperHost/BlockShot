...
public class RecordingHandler {
    ...
    private static void drawRecordingIndicator(int x, int y) {
        ...
        drawRect(poseStack, x, y, maxWidth + 6 + recordOffset, height, 0xb0, 0x10, 0x10, 0x10);
        ...
        if (System.currentTimeMillis() % 2000 > 1000 && getEncoder().showRecordIcon()) {
            drawRect(poseStack, x + 3, y + 4, 7, 5, 0xFF, 0xFF, 0x00, 0x00);
            drawRect(poseStack, x + 4, y + 3, 5, 7, 0xFF, 0xFF, 0x00, 0x00);
        }

        RenderSystem.disableBlend();
    }

    private static void drawRect(PoseStack poseStack, int x, int y, int width, int height, int a, int r, int g, int b) {
        Matrix4f matrix4f = poseStack.last().pose();
        BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
        bufferBuilder.begin(GL11.GL_QUADS, DefaultVertexFormat.POSITION_COLOR);
        bufferBuilder.vertex(matrix4f, x, y + height, 0).color(r, g, b, a).endVertex();
        bufferBuilder.vertex(matrix4f, x + width, y + height, 0).color(r, g, b, a).endVertex();
        bufferBuilder.vertex(matrix4f, x + width, y, 0).color(r, g, b, a).endVertex();
