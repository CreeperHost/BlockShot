...
public class BlockShotGui implements GuiProvider {
    ...
    public void buildGui(ModularGui gui) {
        ...
        gui.setGuiTitle(Component.translatable("gui.blockshot.title"));
        ...
        if (!BlockShot.isActive()) {
            title.setText(Component.translatable("gui.blockshot.history.not_in_offline").withStyle(ChatFormatting.RED));
            return;
        }
        ...
        GuiElement<?> previous;
        //Owner Setting
        previous = new GuiText(root, Component.translatable("gui.blockshot.settings.upload.owner").withStyle(ChatFormatting.UNDERLINE))
        ...
        previous = Flat.button(root, Component.empty())
                .setTooltipSingle(Component.translatable("gui.blockshot.settings.upload.info"))
                .onPress(() -> {
                ...
                .constrain(LEFT, relative(historyPanel.get(RIGHT), 12))
                .constrain(HEIGHT, literal(14));
        ((GuiButton) previous).getLabel().setTextSupplier(() -> Config.INSTANCE.anonymous ? Component.translatable("gui.blockshot.settings.upload.anonymous") : gui.mc().player.getName());

        //Upload Setting
        previous = new GuiText(root, Component.translatable("gui.blockshot.settings.upload_mode").withStyle(ChatFormatting.UNDERLINE))
                .constrain(RIGHT, relative(root.get(RIGHT), -4))
...
//        new GuiLoadingSpinner(root, new ResourceLocation(BlockShot.MOD_ID, "textures/gui/blockshot.png"))
    ...
    private GuiElement<?> createHistoryPanel(GuiElement<?> root, int entryWidth) {
        ...
        Component title = Component.translatable("gui.blockshot.history.title").withStyle(ChatFormatting.UNDERLINE);
        Component anonWarning = Component.translatable("gui.blockshot.history.anon_warning").withStyle(ChatFormatting.RED);
        new GuiText(container, () -> Component.empty().append(title).append(Config.INSTANCE.anonymous ? Component.literal(" ").append(anonWarning) : Component.empty()))
        ...
        GuiText noHistory = new GuiText(container, Component.translatable("gui.blockshot.history.no_history"))
        ...
        for (Capture capture : manager.getCaptureHistory()) {
            ...
            if (day != currentDay) {
                ...
                if (day == TimeUnit.MILLISECONDS.toDays(System.currentTimeMillis() + TimeZone.getDefault().getRawOffset())) {
                    dateText = Component.translatable("gui.blockshot.history.today");
                } else {
                    dateText = Component.literal(DAY_OF_MONTH_FORMAT.format(capture.created() * 1000));
                }
